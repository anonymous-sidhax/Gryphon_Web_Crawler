    ...
    if request.method == "POST":
        ...
        try:
            page = requests.get(base_url, headers=HEADERS,
                                allow_redirects=True, timeout=2)  # Get Page
        ...
        try:
            ...
            URLS = [url for element, attribute, url, pos in iterlinks(
                resolve_base_href(make_links_absolute(page.content, base_url)))]
        ...
        try:
            ...
            if expire > now:
                ...
                SUMMARY.update(
                    {0: ["SSL Certificate Expired", base_url, "SSL Expiry: " + expire, ["-"]]})
                Summary_count += 1
        except Exception as e:
            print(e)
            SUMMARY.update(
                {0: ["SSL Error", base_url, "No SSL detected", ["-"]]})
            Summary_count += 1
        ...
        for link in LINKS:
            ...
            duplicate_id_check = Accessibility.duplicate_id_check(
                page.content, link)
            blank_body_check = Errors.blank_body_check(page.content, url)
            meta_description_tag_check = Search.meta_description_tag_check(
                page.content, link)
            title_tag_check = Search.title_tag_check(page.content, link)
            underlined_text_is_not_a_link_check = Usability.underlined_text_is_not_a_link_check(
                page.content, link)
            ...
            if Usability.underlined_text_is_not_a_link_check is not None:
                USABILITY.update(
                    {Usability_count: underlined_text_is_not_a_link_check})
                Usability_count += 1
            ...
            try:
                ...
                if expire > now:
                    ...
                    SUMMARY.update(
                        ["SSL Error", link, "SSL Certificate Expired", ["-"]])
            except Exception as e:
                print(e)
                SUMMARY.update(
